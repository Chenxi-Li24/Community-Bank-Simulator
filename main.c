...
static void manage_pin(Account accounts[], int count) {
    int selected_account_index = -1;
    char entered_password[64];
    char new_pin[7];
    bool account_found = false;

    // Print all account IDs and usernames
    printf("-----Account List-----\n");
    for (int i = 0; i < count; i++) {
        printf("%d. Account ID: %s, Username: %s\n", i + 1, accounts[i].account_id, accounts[i].username);
    }
    // Prompt the user to select an account by index
    printf("\nSelect an account by number: ");
    scanf("%d", &selected_account_index);

    // Validate selection
    if (selected_account_index < 1 || selected_account_index > count) {
    ...
    // Prompt for the password of the selected account
    printf("Enter Password for Account ID: %s: ", accounts[selected_account_index].account_id);
    clearBuffer();
    scanf("%63s", entered_password); // Limit input to avoid overflow
    clearBuffer();
    // Verify password
    if (strcmp(entered_password, accounts[selected_account_index].password) == 0) {
        // Prompt for the new PIN
        printf("Password verified. Enter new PIN: ");
        scanf("%6s", new_pin); // Limit input to 6 characters
        strcpy(accounts[selected_account_index].Pin, new_pin); // Update the PIN
        printf("PIN successfully updated for account ID %s.\n", accounts[selected_account_index].account_id);
        clearBuffer();
    } else {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            if (account_count == 0) {
               ...
               manage_pin(accounts, account_count);
            }
        } else if (choice == 5) {
            printf("Goodbye.\n");
            break;
